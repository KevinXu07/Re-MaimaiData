package com.kevinxu.remaidata.widgets
...
object Settings {
    ...
    fun setEnableAliasSearch(enabled: Boolean) {
        settingsPre.edit().putBoolean(KEY_ALIAS_SEARCH, enabled).apply()
    }

    fun getEnableCharterSearch() =
        settingsPre.getBoolean(KEY_CHARTER_SEARCH, DEFAULT_CHARTER_SEARCH)

    fun setEnableCharterSearch(enabled: Boolean) {
        settingsPre.edit().putBoolean(KEY_CHARTER_SEARCH, enabled).apply()
    }

    fun getEnableShowAlias() =
        settingsPre.getBoolean(KEY_SHOW_ALIAS, DEFAULT_SHOW_ALIAS)

    fun setEnableShowAlias(enabled: Boolean) {
        settingsPre.edit().putBoolean(KEY_SHOW_ALIAS, enabled).apply()
    }

    fun getEnableDivingFishNickname() =
        settingsPre.getBoolean(KEY_USE_DIVING_FISH_NICKNAME, DEFAULT_USE_DIVING_FISH_NICKNAME)

    fun setEnableDivingFishNickname(enabled: Boolean) {
        settingsPre.edit().putBoolean(KEY_USE_DIVING_FISH_NICKNAME, enabled).apply()
    }
    ...
    fun setNickname(nickname: String) {
        settingsPre.edit().putString(KEY_NICKNAME, nickname).apply()
    }
    ...
    fun setUpdateDifficulty(selected: Set<Int>) {
        settingsPre.edit().putStringSet(
            KEY_SELECT_DIFFICULTIES,
            selected.map(Int::toString).toSet()
        ).apply()
    }
}
